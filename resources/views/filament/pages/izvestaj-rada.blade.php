<x-filament-panels::page>
    <style>
        ...
        .ir-toolbar-left { display: flex; align-items: center; gap: .5rem; }
        .ir-month { font-size: 1.05rem; font-weight: 700; color: #1F2937; margin-left: .5rem; text-transform: capitalize; }
        .dark .ir-month { color: #F3F4F6; }

        .ir-summary { display: grid; grid-template-columns: repeat(auto-fit, minmax(10rem, 1fr)); gap: .75rem; }
        .ir-sum-card {
            background: #fff; border: 1px solid #E5E7EB; border-radius: .75rem; padding: .9rem 1.1rem;
        }
    ...
    <div class="ir-toolbar">
        <div class="ir-toolbar-left">
            <x-filament::icon-button color="gray" wire:click="previousMonth" icon="heroicon-o-chevron-left" label="Prethodni mesec" />
            <x-filament::button color="gray" size="sm" wire:click="currentMonth">Ovaj mesec</x-filament::button>
            <x-filament::icon-button color="gray" wire:click="nextMonth" icon="heroicon-o-chevron-right" label="Sledeći mesec" />
            <span class="ir-month">{{ $monthLabel }}</span>
        </div>
    ...
    <div class="ir-summary">
        <div class="ir-sum-card">
            ...
            <div class="ir-sum-value">{{ $total['zavrseno'] }}</div>
        </div>
        <div class="ir-sum-card">
            <div class="ir-sum-label">Ukupan prihod</div>
            <div class="ir-sum-value green">{{ number_format($total['prihod'], 0, ',', '.') }} RSD</div>
        </div>
        <div class="ir-sum-card">
        ...
        <div class="ir-doc">
            <div class="ir-doc-head">
                ...
                <span class="ir-doc-stats">
                    ...
                    <span>prihod: <b>{{ number_format($row['prihod'], 0, ',', '.') }} RSD</b></span>
            ...
            @if ($row['services'] === [])
                <div class="ir-empty">Nema završenih pregleda u ovom mesecu.</div>
            @else
                <table class="ir-table">
                    <thead>
                        <tr>
                            <th>Usluga</th>
                            <th class="ir-num">Broj</th>
                            <th class="ir-num">Cena</th>
                            <th class="ir-num">Ukupno</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($row['services'] as $s)
                            <tr>
                                ...
                                <td class="ir-num">{{ number_format($s['price'], 0, ',', '.') }} RSD</td>
                                <td class="ir-num">{{ number_format($s['total'], 0, ',', '.') }} RSD</td>
                            </tr>
                        @endforeach
                        <tr class="ir-total-row">
                            ...
                            <td></td>
                            <td class="ir-num">{{ number_format($row['prihod'], 0, ',', '.') }} RSD</td>
                        </tr>
                    </tbody>
                </table>
            @endif
